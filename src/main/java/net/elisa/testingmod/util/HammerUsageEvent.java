package net.elisa.testingmod.util;
...
public class HammerUsageEvent implements PlayerBlockBreakEvents.Before{
    ...
    public boolean beforeBlockBreak(World world, PlayerEntity player, BlockPos pos,
        ...
        if(mainHandItem.getItem() instanceof HammerItem hammer && player instanceof ServerPlayerEntity serverPlayer) {
            ...
            int HUNGER_STATE = player.getHungerManager().getFoodLevel();
            int DURABILITY = mainHandItem.getDamage();
            
            if(!hammer.isCorrectForDrops(mainHandItem, world.getBlockState(pos))) {
                return true;
            }
            ...
            for(BlockPos position : HammerItem.getBlocksToBeDestroyed(2, pos, serverPlayer)) {
                ...
                if (player.getGameMode() == GameMode.SURVIVAL) {
                    player.getHungerManager().setFoodLevel(HUNGER_STATE);
                    mainHandItem.setDamage(DURABILITY);
                }
